...
class TestUserRegistration:
    ...
    @allure.title('Проверка регистрация пользователя, который уже существует')
    ...
    @pytest.mark.parametrize(
        "fields_to_pass",
        RegisterUser.test_register_user_no_data_cases
    )
    @allure.title('Проверка регистрация пользователя без необходимых данных')
    @allure.description('Регистрируем пользователя без имени')
    def test_register_user_no_data(self, generate_user_data, fields_to_pass):
        # Arrange
        api = generate_user_data[0]
        email = generate_user_data[1]["email"]
        password = generate_user_data[1]["password"]
        name = generate_user_data[1]["name"]
        fields = fields_to_pass.split(",")
        arguments = {}
        if "email" in fields:
        ...
        # Act
        response = api.create_user(**arguments)
